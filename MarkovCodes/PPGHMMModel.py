...
ECG = pd.read_csv(r"out\PPG-2.csv",header=None)
ECGLabel = pd.read_csv(r"out\PPGLabel-2.csv",header=None)
...
def getEmissionProb(observations,labels,states):
    ...
    for state in states:
        ...
        clf = NormalDistribution(np.mean(stateValues), np.std(stateValues))
